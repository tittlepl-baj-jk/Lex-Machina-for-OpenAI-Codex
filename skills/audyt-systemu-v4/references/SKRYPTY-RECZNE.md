...
| `check_wydanie.py` | Uruchamiany także RĘCZNIE **po każdym pakowaniu** (orkiestrator robi to przy pełnym przebiegu). Rozjazd oznacza, że drzewo zmieniło się po wydaniu — wtedy porównaj z dziennikiem i ustal autorstwo zmiany (AUDYT-2026-09-17p). | Po `zip`; przed zgłoszeniem wydania jako zakończonego. |
| `check_podmiana_aktu.py` | **T31 — wymaga sieci (API ELI).** Wynik zależy od dostępności ELI, nie tylko od repozytorium; kandydaci wymagają oceny (nazwa potoczna → wpis do `ALIASY-NAZW-AKTOW.md` po odczycie tytułu, podmiana → korekta numeru). Dodany 2026-09-16d. | Koniec sesji zmieniającej ROUTING-MAP lub MAPA-AKTOW; razem z T15. |
| `check_widmowe_pokrycie.py` | **T5 — wynik to lista kandydatów, nie błędów.** Wiersze meta (rejestracja current-state, moduły przekrojowe) wychodzą zawsze; rozstrzygnięcie wymaga odczytu modułu i numeru w ELI. Dodany 2026-09-16b. | Koniec sesji zmieniającej ROUTING-MAP lub moduły; `--selftest` przed użyciem. |
| `weryfikator_sygnatur.py` | **V-SYG-0 — narzędzie wykonawcze, nie test regresyjny.** Wymaga sieci (kanały maszynowe orzecznictwa); wynik zależy od dostępności portali, nie od repozytorium. Zarejestrowany 2026-09-16 (F-189, T22). | Sesja z weryfikacją sygnatur; `shared/SYGNATURY.md`. |
| `check_status_podstaw.py` | **T27 — wymaga ŻYWEGO API ELI** (338 numerów na przebieg) oraz **ludzkiego przeglądu wyniku**. Raportuje „DO PRZEGLĄDU", nie FAIL, bo heurystyka tego badania dwukrotnie zawyżyła wynik; automatyczne traktowanie listy jako błędów powielałoby fałszywe alarmy. Tryb `--offline` z cache pozwala powtórzyć przebieg bez sieci. | Sesja audytowa przed wydaniem oraz po każdej większej edycji podstaw prawnych. |
