# mod-ustawa-cudzoziemcy-zatrudnianie [PERSPEKTYWA ADMINISTRACYJNA]

**Status:** moduł uzupełniający do `mod-ustawa-cudzoziemcy.md` (DR-05, moduł kanoniczny)
**Wydzielony:** 2026-06-14 (audyt — moduł nadrzędny >400 linii, podział tematyczny)
**Źródło weryfikacji:** Ustawa o warunkach dopuszczalności powierzania pracy
cudzoziemcom — Dz.U. 2025 poz. 621 (w życie 01.06.2025)
**Data weryfikacji online:** 2026-08-22 (sekcja 0 — dwa rozporządzenia wykonawcze MSZ/MRPiPS dot. Kolumbii, Gruzji, Wenezueli)
**Zasada:** Każde brzmienie przepisu przed powołaniem → isap.sejm.gov.pl
**⚠️ Przepisy imigracyjne są bardzo często nowelizowane — ZAWSZE weryfikuj aktualne brzmienie.**

---

## ⚡ ALERT LEGISLACYJNY

```
Nowa ustawa o pracy cudzoziemców (Dz.U. 2025 poz. 621) — w życie 01.06.2025:
  → Zastąpiła przepisy o zezwoleniach na pracę z ustawy o promocji zatrudnienia
  → Wnioski i oświadczenia: WYŁĄCZNIE ELEKTRONICZNIE przez praca.gov.pl
  → Kary dla pracodawcy: min. 3 000 zł / nielegalnie zatrudnioną osobę, max 50 000 zł łącznie

⭐⭐⭐ NOWE 2026-08-22: KOLUMBIA, GRUZJA, WENEZUELA — koniec ruchu bezwizowego
  jako podstawy pracy (patrz sekcja 0 niżej, opracowana w pełni tego dnia)
```

---

## 0. ⭐⭐⭐ WYŁĄCZENIE RUCHU BEZWIZOWEGO DLA KOLUMBII, GRUZJI I WENEZUELI —
DWA ODRĘBNE ROZPORZĄDZENIA (dodano 2026-08-22)

✅ **ZWERYFIKOWANE RZĄD 1+2B, wielokrotnie zgodne źródła.** Zmiana opiera się
na DWÓCH odrębnych rozporządzeniach wykonawczych, wydanych przez dwóch
różnych ministrów, z różnymi datami wejścia w życie — nie mylić ich ze sobą.

```
⭐⭐⭐ PODSTAWA USTAWOWA (art. 3 ust. 4-6 ustawy Dz.U. 2025 poz. 621):
  Art. 3 ust. 5 pkt 6 — cudzoziemiec uprawniony do pobytu W RAMACH RUCHU
    BEZWIZOWEGO może wykonywać pracę BEZ zezwolenia na pracę/oświadczenia,
    CHYBA że jest obywatelem państwa z wykazu, o którym mowa w ust. 4.
  Art. 3 ust. 4 — minister właściwy ds. pracy, NA WNIOSEK ministra ds.
    wewnętrznych LUB ministra ds. zagranicznych, MOŻE określić w drodze
    ROZPORZĄDZENIA wykaz państw, których obywatele z zezwoleniem na pracę
    NIE SĄ uprawnieni do jej wykonywania w ramach ruchu bezwizowego. W
    rozporządzeniu uwzględnia się zagrożenia bezpieczeństwa i porządku
    publicznego, w tym wysokie ryzyko wjazdu do strefy Schengen POD
    POZOREM wykonywania pracy w Polsce.

⭐⭐⭐ ROZPORZĄDZENIE #1 — MRPiPS, PRACA (Dz.U. 2026 poz. 1072):
  Rozporządzenie Ministra Rodziny, Pracy i Polityki Społecznej z dnia
  31 lipca 2026 r. w sprawie wykazu państw, których obywatele posiadający
  zezwolenie na pracę nie są uprawnieni do wykonywania pracy w czasie
  pobytu na terytorium RP w ramach ruchu bezwizowego.
  → WYKAZ (art. 1 rozporządzenia — katalog ZAMKNIĘTY, 3 państwa):
    1) Boliwariańska Republika Wenezueli
    2) Gruzja
    3) Republika Kolumbii
  → WEJŚCIE W ŻYCIE: **22 sierpnia 2026 r.** (ogłoszone 7.08.2026, 14 dni
    vacatio legis)
  → SKUTEK PRAKTYCZNY: obywatele tych 3 państw, których podstawą pobytu
    jest WYŁĄCZNIE ruch bezwizowy, NIE MOGĄ już wykonywać pracy nawet
    posiadając zezwolenie na pracę — sam ruch bezwizowy przestaje
    wystarczać jako TYTUŁ POBYTOWY uprawniający do pracy. Wymagana jest
    ODRĘBNA podstawa pobytowa (wiza, karta pobytu) NIEZALEŻNIE od
    posiadania zezwolenia na pracę.
  → ⭐ PRZEPIS PRZEJŚCIOWY: cudzoziemcy, którzy ROZPOCZĘLI legalną pracę
    PRZED 22.08.2026, objęci są okresem przejściowym (dokładne warunki —
    patrz treść rozporządzenia, punkt do pogłębienia w kolejnej sesji).

⭐⭐⭐ ROZPORZĄDZENIE #2 — MSZ, WJAZD (Dz.U. 2026 poz. 1085 i poz. 1086):
  Dwa rozporządzenia Ministra Spraw Zagranicznych z dnia 10 sierpnia
  2026 r. — ustanowienie wyjątków od zwolnienia z obowiązku wizowego
  wobec cudzoziemców, o których mowa w art. 6 ust. 1 lit. a (poz. 1085)
  oraz art. 6 ust. 3 (poz. 1086) rozporządzenia Parlamentu Europejskiego
  i Rady (UE) 2018/1806 (wykaz państw trzecich objętych/zwolnionych z
  obowiązku wizowego przy przekraczaniu granic zewnętrznych UE).
  → SKUTEK: obywatele Gruzji, Republiki Kolumbii oraz Boliwariańskiej
    Republiki Wenezueli, przy próbie WJAZDU na terytorium RP, muszą
    LEGITYMOWAĆ SIĘ WIZĄ (odrębnie od kwestii pracy — dotyczy każdego
    celu podróży objętego wyjątkiem, nie tylko pracy).
  → ⭐⭐ PRZY KAŻDYM WNIOSKU WIZOWYM: właściwe organy konsularne mają
    obowiązek SZCZEGÓŁOWO OCENIAĆ przesłanki RYZYKA MIGRACYJNEGO
    dotyczące KAŻDEGO cudzoziemca z osobna (indywidualna ocena, nie
    automatyzm).
  → OGŁOSZENIE: 12 sierpnia 2026 r. — WESZŁO W ŻYCIE Z DNIEM OGŁOSZENIA
    (12.08.2026), faktycznie stosowane od ok. 15-16.08.2026 wg
    doniesień prasowych.
  → ⭐ PRZEPIS PRZEJŚCIOWY: obywatele tych 3 państw, którzy PRZEKROCZYLI
    granicę Polski PRZED wejściem rozporządzenia w życie, pozostają na
    DOTYCHCZASOWYCH zasadach (dotychczasowe zwolnienie wizowe).

⭐⭐⭐ DWIE RÓŻNE DATY — ⛔ PUŁAPKA PRAKTYCZNA:
  Rozporządzenie MSZ (wjazd/wiza) — w mocy od ok. 12-16.08.2026
  Rozporządzenie MRPiPS (praca) — w mocy od 22.08.2026
  → W OKRESIE PRZEJŚCIOWYM między obiema datami istniało krótkie okno,
    w którym obywatel jednego z trzech państw MÓGŁ jeszcze wjechać bez
    wizy (jeśli przekroczył granicę przed 12-16.08), a mimo to legalnie
    pracować na dotychczasowych zasadach ruchu bezwizowego — do
    22.08.2026. Od 22.08.2026 obie bariery działają łącznie: bez wizy
    NIE MA wjazdu w celu pracy, a nawet przy posiadanej już podstawie
    pobytowej z ruchu bezwizowego sprzed 22.08 sam fakt tego pobytu
    NIE uprawnia do podjęcia/kontynuowania pracy bez odrębnego tytułu.

⭐⭐ KONTEKST STATYSTYCZNY (dla oceny skali praktycznej): na koniec
  czerwca 2026 r. Kolumbia — 23 177 ubezpieczonych w ZUS (4. najliczniejsza
  narodowość zagraniczna), Gruzja — 22 118 (5. miejsce). Uzasadnienie
  MSWiA/MSZ: systemowe nadużywanie ruchu bezwizowego do pracy z
  pominięciem procedur, szara strefa, wyzysk, handel ludźmi.

⭐ KRYTYKA ŚRODOWISKA BIZNESOWEGO (kontekst, nie treść normatywna):
  Konfederacja Lewiatan i inne organizacje pracodawców wskazują ryzyko
  pogłębienia niedoboru kadr — w I półroczu 2026 r. wydano ok. 69 tys.
  nowych zezwoleń na pracę, spadek o ponad 50% r/r. Urzędy ds.
  cudzoziemców uznawane za niedofinansowane, z wielomiesięcznym czasem
  oczekiwania na decyzje.
```

**Weryfikacja (metodologia ZASADA 14):** RZĄD 1 — lexlege.pl/isap.sejm.gov.pl
(pełny tekst art. 3 ustawy, dziennikustaw.gov.pl PDF D2025000062101.pdf),
gov.pl/web/kolumbia (placówka dyplomatyczna RP, cytuje wprost treść
rozporządzenia MRPiPS z numerem Dz.U.), rp.pl (Dziennik Ustaw z 12.08.2026,
poz. 1084-1088 — potwierdza numery 1085/1086 rozporządzeń MSZ). RZĄD 2B —
wielokrotnie zgodne relacje prasowe (pulshr.pl, wp.pl, se.pl, biznesinfo.pl,
kresy.pl, innpoland.pl, stronymonki.pl) — wszystkie niezależnie potwierdzają
te same daty, numery i mechanizm.

---

## Zakres modułu

Moduł główny (`mod-ustawa-cudzoziemcy.md`) pokrywa: tytuły pobytowe (taksonomia
wiz i kart pobytu), procedurę odwoławczą UW → SZUSC → WSA → NSA, wydalenie,
ochronę międzynarodową, ochronę tymczasową (Ukraina).

Ten moduł pokrywa **zezwolenia na pracę cudzoziemców** (ustawa Dz.U. 2025 poz. 621):

0. Wyłączenie ruchu bezwizowego dla Kolumbii, Gruzji i Wenezueli (2026-08-22)
1. Pełna taksonomia zezwoleń na pracę (typy A/B/C/D/S + oświadczenie PUP)
2. Matryca: dokument pobytowy → uprawnienie do pracy

---

## 1. ZEZWOLENIA NA PRACĘ — PEŁNA TAKSONOMIA

```
ELEKTRONIZACJA: wszystkie wnioski WYŁĄCZNIE przez praca.gov.pl
Organ: Wojewoda właściwy ze względu na siedzibę pracodawcy (zezwolenia A–D)
       Starosta (zezwolenie sezonowe S)

TYPY ZEZWOLEŃ (ustawa Dz.U. 2025 poz. 621 — weryfikuj aktualny tekst w ISAP):

Typ A — ogólne zezwolenie na pracę (art. 26):
  → Cudzoziemiec zatrudniony u polskiego pracodawcy na umowę o pracę / cywilnoprawną
  → Wniosek składa pracodawca do WUW (elektronicznie — praca.gov.pl)
  → Test rynku pracy: obowiązkowy, chyba że zawód na liście zwolnień
  → Ważność: do 3 lat; zmiana pracodawcy → nowe zezwolenie
  → Zmiana nie wymagana gdy: zmiana nazwy pracodawcy, przejście zak. pracy, +wymiar czasu pracy

Typ B — pełnienie funkcji w zarządzie spółki (art. 35):
  → Członek zarządu, prokurent lub kurator spółki prawa handlowego
  → Brak testu rynku pracy
  → Organ: Wojewoda

Typ C — praca dla zagranicznego pracodawcy delegowanego do PL (art. 40):
  → Cudzoziemiec delegowany przez podmiot zagraniczny do pracy na terenie PL
  → Wniosek składa podmiot zagraniczny / przejmujący pracę
  → Brak testu rynku pracy

Typ D — praca dla podmiotu zagranicznego, wykonywana poza PL przez wyjazdy służbowe (art. 26):
  → Cudzoziemiec zamieszkały w PL, wyjeżdżający do siedziby pracodawcy za granicą
  → Rzadziej stosowany

Typ S — sezonowe zezwolenie na pracę (art. 45–60):
  → Dla sektorów: rolnictwo, ogrodnictwo, turystyka (wykaz w rozporządzeniu)
  → Ważność: do 9 miesięcy w roku kalendarzowym
  → Organ: STAROSTA (nie Wojewoda!)
  → Wniosek przez praca.gov.pl; możliwość wielosezonowości
  → Zmiana nie wymagana przy: zwiększeniu wymiaru czasu pracy, zmianie nazwy pracodawcy

OŚWIADCZENIE O POWIERZENIU PRACY (art. 61–71 ustawy 621):
  → Dla obywateli wybranych państw: UA, BY, GE, MD, AM, RU (+ inne — weryfikuj rozp.)
  → Rejestracja w PUP (nie WUW) — przez praca.gov.pl
  → Max 24 miesiące w ciągu 36 miesięcy
  → Praca może rozpocząć się max 12 mies. od wpisu oświadczenia (6 mies. — z PL)
  → NIE dotyczy wiz w celach: 01, 02, 11, 14, 20, 23 i tranzytowych

JEDNOLITE ZEZWOLENIE NA POBYT CZASOWY I PRACĘ (art. 114 u.c.):
  → Łączy zezwolenie pobytowe i pracę w jednej decyzji Wojewody
  → Wydaje Wojewoda (nie WUW) na wniosek pracodawcy lub cudzoziemca
  → Nowy obowiązek od 01.06.2025: pracodawca zawiadamia Wojewodę w 15 dni rob. o utracie pracy
  → Zmiana pracodawcy bez zmiany zezwolenia: NIE (co do zasady — nowa ustawa)

NIEBIESKA KARTA UE (art. 127 u.c.) — zmiany od 01.06.2025:
  → Dla zawodów wymagających wysokich kwalifikacji:
     • wykształcenie wyższe (studia) LUB 5 lat dośw. w zawodzie na równorzędnym poziomie
     • Wynagrodzenie brutto ≥ 150% przeciętnego wynagrodzenia poprzedniego roku
       (dla wniosków z 2025 r. ≈ 12 272 zł brutto/mies.)
  → Ważność: do 3 lat
  → Od 01.06.2025: PEŁNY dostęp do rynku pracy (zmiana pracodawcy bez zmiany zezwolenia)
     Obowiązek: zawiadomienie Wojewody o zmianie pracodawcy (pod rygorem cofnięcia)
  → Karty wydane PRZED 01.06.2025: swoboda zmiany pracy dopiero po 2 latach

ICT — PRZENIESIENIE WEWNĄTRZ PRZEDSIĘBIORSTWA (art. 139a u.c.):
  → Dla: kadry kierowniczej, specjalistów, pracowników odbywających staż
  → Pracodawca macierzysty poza UE/EOG/CH → przeniesienie do jednostki przyjmującej w PL
  → Wymogi: powiązanie kapitałowe pracodawca macierzysty ↔ jednostka przyjmująca
  → Wniosek składa jednostka przyjmująca w PL (nie cudzoziemiec)
  → Ważność: do 3 lat
  → Wiza D z celem pkt 24a lub karta pobytu ICT

KARY DLA PRACODAWCY za nielegalne zatrudnienie:
  → Min. 3 000 zł za każdą nielegalnie zatrudnioną osobę
  → Max 50 000 zł łącznie
  → Inspektorzy PIP: mandat do 10 000 zł
```

---

## 2. MATRYCA: DOKUMENT POBYTOWY → UPRAWNIENIE DO PRACY

> ⚠️ Weryfikuj każdorazowo w ISAP (art. 3 ustawy Dz.U. 2025 poz. 621) — lista może być rozszerzana rozporządzeniem.

| Dokument / tytuł pobytowy | Praca bez zezwolenia? | Uwagi |
|---|---|---|
| Obywatel UE / EOG / Szwajcaria | TAK | Swoboda świadczenia usług / pracy |
| Członek rodziny obywatela UE/EOG/CH | TAK | Na podstawie prawa UE |
| Karta pobytu stałego (art. 195 u.c.) | TAK | Pełny dostęp do rynku pracy |
| Rezydent długoterminowy UE (art. 211 u.c.) | TAK | Pełny dostęp |
| Status uchodźcy | TAK | Art. 3 ust. 1 ustawy 621 |
| Ochrona uzupełniająca | TAK | Art. 3 ust. 1 ustawy 621 |
| Pobyt tolerowany | TAK | Art. 3 ust. 1 ustawy 621 |
| Ochrona czasowa (PESEL UKR) | TAK (powiadomienie PUP) | Dz.U. 2022 poz. 583; weryfikuj termin obowiązywania |
| Karta Polaka | TAK | Zwolnienie z zezwolenia na pracę |
| Absolwent polskiej uczelni (studia dzienne) | TAK | Dyplom polskiej szkoły wyższej |
| Karta pobytu — Niebieska Karta UE (po 01.06.2025) | TAK | Pełny dostęp; zmiana pracodawcy z zawiadomieniem |
| Karta pobytu — jednolite zezwolenie pobyt+praca (art. 114) | TAK (u wskazanego pracodawcy) | Zmiana pracodawcy → nowe zezwolenie |
| Karta pobytu — pobyt czasowy w celu studiów (art. 144) | TAK (bez limitu godzin dla dziennych) | Reforma Dz.U. 2025 poz. 622 |
| Karta pobytu — pobyt czasowy w celu badań (art. 151b) | TAK (badania) | — |
| Karta pobytu ICT (art. 139a) | TAK (jednostka przyjmująca) | Nie zmienia pracodawcy swobodnie |
| Wiza D cel pkt 5 (oświadczenie) | TAK (u wskazanego pracodawcy, do 24 mies./36) | Oświadczenie wpisane do ewidencji PUP |
| Wiza D cel pkt 5a (sezonowe) | TAK (sektory sezonowe) | Zezwolenie sezonowe Starosty |
| Wiza D cel pkt 6 (zezwolenie na pracę typ A/C) | TAK (u wskazanego pracodawcy) | Zezwolenie Wojewody |
| Wiza D cel pkt 9 (studia) | TAK (bez zezwolenia — studenci dzienni) | — |
| Wiza D cel pkt 12/13 (dydaktyczny/badania) | TAK (zakres zezwolenia) | — |
| Wiza C Schengen (jakikolwiek cel) | NIE | Zakaz pracy |
| Wiza D cele 01, 02, 11, 14, 20, 23, tranzyt | NIE | Od 01.06.2025 explicite wyłączone |
| Pobyt nielegalny / po upływie tytułu | NIE | Kara dla pracodawcy min. 3 000 zł/osobę |

> **Reguła praktyczna dla pracodawcy:** Przed zatrudnieniem skopiuj i sprawdź:
> 1. Dokument tożsamości cudzoziemca (paszport / dowód UE)
> 2. Tytuł pobytowy (wiza z celem / karta pobytu z adnotacją)
> 3. Zezwolenie na pracę / oświadczenie PUP (o ile wymagane)
> 4. Aktualność (daty ważności wszystkich dokumentów)
> Kopie przechowuj w aktach pracownika.

---

## POWIĄZANIA

| Sytuacja | Skill / Moduł |
|---|---|
| Tytuły pobytowe, taksonomia wiz/kart pobytu | `mod-ustawa-cudzoziemcy.md` |
| Procedura odwoławcza, wydalenie, ochrona międzynarodowa | `mod-ustawa-cudzoziemcy.md` |
| Prawo pracy — strona pracownicza | `dr-04` |
| Kary PIP, nielegalne zatrudnienie | `dr-04` → moduł PIP/inspekcja pracy |

---

## ŹRÓDŁA ONLINE

- Ustawa o pracy cudzoziemców: https://isap.sejm.gov.pl/isap.nsf/DocDetails.xsp?id=WDU20250000621
- praca.gov.pl: https://praca.gov.pl
- Urząd ds. Cudzoziemców: https://udsc.gov.pl

```
web_search: "ustawa praca cudzoziemców Dz.U. 2025 poz. 621 elektronizacja isap"
```

---

## ⚖️ DISCLAIMER

Po zakończeniu analizy: `view ../../shared/DISCLAIMER.md` — wariant wg trybu (PRAWNIK/LAIK).
