# Moduł — Prawo geologiczne i górnicze

## Akt prawny / źródło
- Ustawa Prawo geologiczne i górnicze — Dz.U. 2026 poz. 69 t.j. (POPRAWKA
  2026-07-02j — było błędnie 2024.1290; obwieszczenie 9.01.2026)
  ✅ [VER: isap.sejm.gov.pl, 2026-05-31 — metryka]
  ✅ FAZA 3E ZAMKNIĘTA 2026-08-20 (F-40): sprawdzono 3 nowelizacje między
  starym a nowym t.j. (Dz.U. 2025 poz. 769 — ustawa deregulacyjna, poz.
  1023 — WYŁĄCZNIE korekta odnośnika do tytułu ustawy/dyrektyw UE
  transponowanych, BEZ zmian merytorycznych; poz. 1668 — dot. planowania
  przestrzennego, poza zakresem treści tego modułu) — ŻADNA nie dotyka
  treści opisanej w module (koncesje, własność złóż, opłata
  eksploatacyjna, art. 168 sankcja). Treść modułu POTWIERDZONA jako
  aktualna mimo zmiany numeru t.j.

**Weryfikacja:** isap.sejm.gov.pl + MŚ przed każdym cytowaniem.

## Zakres
Reguluje poszukiwanie, rozpoznawanie i wydobywanie kopalin ze złóż, podziemne
bezzbiornikowe magazynowanie substancji, podziemne składowanie odpadów, roboty
geologiczne.

Koncesje: wydawane przez Ministra Klimatu (kopaliny podstawowe: węgiel kamienny,
brunatny, rudy metali, ropa, gaz) lub Starostę (kopaliny pospolite: piasek, żwir, glina)
lub Marszałka Województwa.

## Zasady absolutne
```
Własność złóż:   Kopaliny podstawowe — własność Skarbu Państwa; kopaliny pospolite — właściciela gruntu
Opłata eksploatacyjna: Zależna od wydobycia — weryfikuj rozp. RM
Samowolne wydobycie:  Kara administracyjna (art. 168 p.g.g.) + przywrócenie do stanu poprzedniego
Organ nadzoru:   Wyższy Urząd Górniczy (WUG) — nadzór nad bezpieczeństwem górniczym
```

## Połącz z
- dr-09-budownictwo-srodowisko-energia-transport/modules/mod-POS-prawo-ochrony-srodowiska.md (OOŚ przy wydobyciu)
- DR-09/mod-PrGeodezyjne-kartografia-wywlaszczenia (kwestie wywłaszczeniowe)

## Weryfikacja online
```
web_search: "prawo geologiczne górnicze Dz.U. 2024 poz. 1290 isap tekst jednolity"
```
