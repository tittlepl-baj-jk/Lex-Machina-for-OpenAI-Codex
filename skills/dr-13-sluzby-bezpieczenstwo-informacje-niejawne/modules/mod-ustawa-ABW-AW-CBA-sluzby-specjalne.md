...
Kontrola operacyjna — postanowienie sądu:
  → Wymagana zgoda sądu okręgowego (na wniosek szefa służby + PG lub MS)
  → Termin: 3 miesiące (przedłużalny)
  → Materiały z kontroli niezgodnej z prawem: niedopuszczalne jako dowód
  → Zniszczenie zebranych materiałów gdy nie użyto w postępowaniu

Skargi na działania służb:
  → Skargi do Kolegium ds. Służb Specjalnych (przy Prezesie RM)
  → Sejmowa Komisja ds. Służb Specjalnych — kontrola parlamentarna
  → RPO — skargi na naruszenia praw obywatelskich
  → Sąd cywilny — odszkodowanie za bezprawne działania (art. 417 KC)
  → Skarga do ETPC — po wyczerpaniu krajowych środków
  ⚠️ Ograniczona jawność postępowania → buduj argumentację na procedurze i skutku
```
