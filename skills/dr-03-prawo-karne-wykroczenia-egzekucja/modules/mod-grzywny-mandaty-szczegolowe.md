# mod-grzywny-mandaty-szczegolowe

**Status:** nowy moduł 2026-06-09
**Zakres:** Grzywny sądowe (KW), mandaty karne, grzywny administracyjne (KPA/UPEA),
kary pieniężne administracyjne (KPA Dział IVa), przedawnienie, egzekucja

**Źródła (weryfikuj w ISAP przed powołaniem):**

| Akt | Dz.U. | Uwaga |
|---|---|---|
| Kodeks wykroczeń (KW) | Dz.U. 2025 poz. 734 t.j. | art. 24–37 (grzywna sądowa) |
| KPSW | Dz.U. 2025 poz. 860 t.j. ze zm. **Dz.U. 2026 poz. 473** | art. 95–102 (mandat karny). ⛔ 2026 poz. 473 to NOWELIZACJA (ustawa o zmianie ustawy o PIP oraz niektórych innych ustaw), **nie** t.j. — wcześniejszy zapis „2026.473 t.j." był błędny, sprostowany 2026-08-15 |
| KPA | Dz.U. 2025 poz. 1691 t.j. | Dział IVa art. 189a–189k (adm. kary pieniężne) |
| KPA art. 88 | j.w. | grzywna porządkowa świadka/biegłego |
| UPEA | Dz.U. 2026 poz. 268 t.j. ze zm. Dz.U. 2026 poz. 739 | art. 119–125 (grzywna w celu przymuszenia). ⛔ POPRAWIONE 2026-08-15: wcześniej moduł podawał 2023 poz. 2505 — numer nieaktualny o DWA cykle t.j. |
| Ustawa o drogach publicznych (UDP) | Dz.U. 2025 poz. 889 t.j. | art. 13, 13b, 13f (opłaty parkingowe) |

---

## 1. SYSTEMATYKA GRZYWIEN I KAR — MAPA TYPÓW

```
TYP A — GRZYWNA SĄDOWA (KW art. 24–37):
  → Orzekana przez sąd (SR) w postępowaniu o wykroczenie
  → Max: 5 000 zł (art. 24 §1 KW — weryfikuj aktualne brzmienie w ISAP)
  ⚠️ UWAGA: od 2022 r. przepis zmieniony — weryfikuj isap.sejm.gov.pl → KW art. 24
  → Możliwa zamiana na pracę społecznie użyteczną (art. 25 KW) lub areszt (art. 26 KW)
  → Tryb: wniosek o ukaranie → rozprawa SR → wyrok → apelacja do SO

TYP B — MANDAT KARNY (KPSW art. 95–102):
  → Nakładany przez: policję, ITD, straż miejską, inne uprawnione organy
  → ⛔ ZASADA OGÓLNA (art. 96 § 1 KPSW, zweryfikowane 2026-08-15):
    • do 500 zł, a przy zbiegu przepisów z art. 9 § 1 KW — do 1 000 zł
    • kwoty 2 500 zł / 5 000 zł podawane niżej NIE wynikają z art. 96 § 1,
      lecz z przepisów SZCZEGÓLNYCH (taryfikator drogowy, recydywa
      w wykroczeniach drogowych) — nie mylić poziomów regulacji
  → KWOTY SZCZEGÓLNE w samym art. 96 KPSW (katalog, stan 2026-08-15):
    • § 1a — sprawy PIP oraz naruszenia z ustawy o transporcie drogowym:
      ⭐ **ZMIANA OD 8.07.2026 (Dz.U. 2026 poz. 473):** dla spraw, w których
      oskarżycielem publicznym jest PIP — **do 5 000 zł** (było: do 2 000 zł);
      dodany pkt dot. art. 84 ustawy z 20.03.2025 o warunkach dopuszczalności
      powierzania pracy cudzoziemcom
    • § 1aa — bezpieczeństwo imprez masowych (art. 54–56, 57a): 2 000 zł
    • § 1ab — monitorowanie przewozu towarów (art. 32 ust. 1): 5 000–7 500 zł
    • drogi publiczne (art. 13na ust. 1, 13naa ust. 1 UDP): 1 500 zł
    • art. 96 § 3 KW (dopuszczenie do prowadzenia pojazdu wbrew przepisom):
      do 8 000 zł
  → STAWKI DROGOWE (weryfikuj aktualny taryfikator — Dz.U. 2026 poz. 724
    lub wcześniejsze):
    • Standardowy: max 2 500 zł
    • Recydywa (ten sam typ w ciągu 24 mies.): max 5 000 zł
  → Mandat gotówkowy: zapłata na miejscu = natychmiastowe uprawomocnienie
  → Mandat bezgotówkowy: karta/instrument płatniczy, o ile funkcjonariusz
    ma terminal (art. 98 § 3a-3c KPSW); ⛔ koszty autoryzacji i przekazu
    ponosi UKARANY, funkcjonariusz ma obowiązek go o tym poinformować
  → Mandat kredytowany: 7 dni na zapłatę
  → ODMOWA mandatu → organ kieruje wniosek o ukaranie do SR
  → UCHYLENIE prawomocnego: art. 101 KPSW — 7 dni ZAWITY (patrz sekcja 3)
  → EGZEKUCJA nieopłaconego: UPEA / US jako organ egzekucyjny

TYP C — ADMINISTRACYJNA KARA PIENIĘŻNA (KPA Dział IVa — art. 189a–189k):
  → Nakładana w trybie administracyjnym decyzją organu administracji
  → Zasady: proporcjonalność, przesłanki wymiaru, dyrektywy (KPA art. 189d — weryfikuj)
  → Termin przedawnienia WSZCZĘCIA postępowania: 5 lat od naruszenia (art. 189g KPA)
  → Termin przedawnienia NAŁOŻENIA: weryfikuj art. 189g–h KPA
  → Tryb: postanowienie/decyzja → odwołanie do organu II stopnia → SKO lub organ wyższy → WSA → NSA
  → Ulgi: odstąpienie, rozłożenie na raty, odroczenie (art. 189k KPA)
  → Weryfikuj: isap.sejm.gov.pl → KPA → Dział IVa (art. 189a–189k)

TYP D — GRZYWNA PORZĄDKOWA (KPA art. 88):
  → Za bezzasadną odmowę zeznań, niestawiennictwo jako świadek/biegły
  → Max: 50 zł (pierwsze); 200 zł (ponowne) — weryfikuj aktualne brzmienie
  → Termin na wniosek o uchylenie: 7 dni od zawiadomienia
  → Zażalenie: na postanowienie o odmowie uchylenia
  ⚠️ Wartości są niskie — ale tryb jest standardowy KPA

TYP E — GRZYWNA W CELU PRZYMUSZENIA (UPEA art. 119–125):
  → Środek egzekucji obowiązków niepieniężnych (znoszenie, zaniechanie, czynności niezastępowalne)
  → Jednorazowo: max 10 000 zł (osoby fizyczne) / 50 000 zł (prawne)
  → Łącznie wielokrotne: max 50 000 zł (fizyczne) / 200 000 zł (prawne)
  → Może być nakładana wielokrotnie w tej samej lub wyższej kwocie
  → Umorzenie: na wniosek zobowiązanego (art. 125 UPEA) po wykonaniu obowiązku
  → Organ: organ egzekucyjny (najczęściej naczelnik US lub wójt/burmistrz)
```

---

## 2. TARYFIKATOR MANDATÓW DROGOWYCH

```
⚠️⚠️ NAPRAWIONO 2026-06-13 — BŁĄD CYTOWANIA "PRAWDZIWY CYTAT W ZŁYM
KONTEKŚCIE": poprzednia wersja podawała "Dz.U. 2026 poz. 724 (MSWiA
29.05.2026)" jako podstawę TARYFIKATORA KWOT. Ten Dz.U. ISTNIEJE
i ISTOTNIE jest z 29.05.2026 — ALE dotyczy ZUPEŁNIE INNEGO przedmiotu:
"rozporządzenie MSWiA z 29.05.2026 r. w sprawie ewidencji kierujących
pojazdami naruszających przepisy ruchu drogowego" (system PUNKTÓW
KARNYCH, weszło w życie 03.06.2026, ogranicza redukcję punktów po
szkoleniu — max raz na 6 miesięcy, tylko dla kierowców z prawem jazdy
>1 rok). NIE zawiera taryfikatora KWOT mandatów.

✅ PRAWIDŁOWA PODSTAWA TARYFIKATORA KWOT (zweryfikowano online 2026-06-13,
   źródła: jazda.io, mubi.pl, gazetaprawna.pl — marzec/maj 2026):
Rozporządzenie Prezesa Rady Ministrów z 30.12.2021 r. w sprawie wysokości
grzywien nakładanych w drodze mandatów karnych za wybrane rodzaje wykroczeń
(Dz.U. 2021 poz. 2484 — weryfikuj t.j. w ISAP)
  → STAN NA 03.2026: "te same kwoty co w 2022 r." — BRAK podwyżek,
    BRAK planów podwyżki (jazda.io, marzec 2026)
  → web_search: "rozporządzenie wysokość grzywien mandaty karne 2026
    tekst jednolity isap.sejm.gov.pl"

DZ.U. 2026 POZ. 724 — UŻYJ DLA WŁAŚCIWEGO TEMATU:
  → System punktów karnych / ewidencja kierowców / redukcja punktów
    po szkoleniu w WORD → DR-03 sekcja punkty karne (jeśli istnieje)
    lub mod-PRD-prawo-jazdy-punkty-karne.md (DR-03)
  → NIE używaj tego Dz.U. dla kwot mandatów

ZASADY MANDATÓW KARNYCH (KPSW — weryfikuj aktualny art. 96 §1–4):
  → Uprawnieni do wystawiania: policja, ITD, straż gminna/miejska, inne upoważnione organy
  → Mandat = jedyny tryb szybkiego zakończenia sprawy
  → Odmowa = obliguje organ do złożenia wniosku o ukaranie do SR
  → RECYDYWA: ten sam typ naruszenia w ciągu 2 lat (24 miesięcy) →
    podwyższona/podwójna kwota, max 5 000 zł (ZWERYFIKOWANO: gazetaprawna.pl
    05.2026 — "recydywa wykroczeniowa" potwierdzona dla najpoważniejszych
    naruszeń prędkości)

STAWKI ORIENTACYJNE — DROGOWE (weryfikuj aktualne przed powołaniem):
┌──────────────────────────────────────────────────────────────────────┐
│ Art.   │ Naruszenie                    │ Mandat     │ Sąd max       │
├────────┼───────────────────────────────┼────────────┼───────────────┤
│ 92a    │ Prędkość: do 10 km/h          │ 50–100 zł  │ 5 000 zł      │
│        │ Prędkość: 11–20 km/h          │ 200–300 zł │               │
│        │ Prędkość: 21–30 km/h          │ 300–400 zł │               │
│        │ Prędkość: 31–40 km/h          │ 800 zł     │               │
│        │ Prędkość: 41–50 km/h          │ 1 000 zł   │               │
│        │ Prędkość: >50 km/h            │ 1 500–2 500│ (5 000 w recyd.)│
│ 86 §1  │ Kolizja (nieostrożność)       │ 500–1 500  │ 5 000 zł      │
│ 87 §1  │ Alkohol 0,2–0,5‰             │ 2 500 zł   │ 5 000 zł      │
│ ⚠️86c  │ "Drift/celowy poślizg" — NIE  │ —          │ —             │
│        │ ZNALEZIONO POTWIERDZENIA tego │            │               │
│        │ artykułu w wyszukiwaniu       │            │               │
│        │ online 2026-06-13. Może być   │            │               │
│        │ błędny numer art. lub zmylenie│            │               │
│        │ z BRD I/II (nowe ustawy       │            │               │
│        │ drogowe z 06.2026 — patrz     │            │               │
│        │ mod-PRD). PRZED UŻYCIEM:      │            │               │
│        │ web_search "art 86c kodeks    │            │               │
│        │ wykroczeń drift" + isap       │            │               │
│ 94 §1  │ Jazda bez uprawnień           │ 1 500 zł   │ 5 000 zł      │
│ 96 §3  │ Nieujawnienie kierowcy        │ 2 500 zł   │ 5 000 zł      │
└────────┴───────────────────────────────┴────────────┴───────────────┘
⚠️ Max grzywna sądowa: weryfikuj aktualny art. 24 §1 KW w ISAP
⚠️ Minimalne kwoty 2026 (gazetaprawna.pl 05.2026, dla "nieujawnienie
   kierującego" art.96§3): postępowanie o przestępstwo → min 4000 zł,
   zagrożenie bezpieczeństwa → min 2000 zł, przekroczenie prędkości →
   min 800 zł, pozostałe → min 500 zł. Te kwoty mogą RÓŻNIĆ SIĘ od
   wartości w tabeli wyżej — zawsze weryfikuj ISAP przed powołaniem.
```

---

## 3. MANDAT — UCHYLENIE I OBRONA

### Odmowa mandatu
```
ODMÓW NA MIEJSCU gdy:
  → Nie jesteś sprawcą / fakty sporne
  → Czyn nie wyczerpuje znamion wykroczenia
  → Upłynął termin przedawnienia (1 rok od czynu)
  → Kwota rażąco zawyżona / błąd co do kwoty
  → Brak pouczenia o prawie odmowy → błąd proceduralny

SKUTEK ODMOWY:
  → Organ sporządza wniosek o ukaranie → SR → tryb nakazowy LUB rozprawa
  → Wyrok nakazowy: SPRZECIW w 7 dni od doręczenia (ZAWITY — art. 94 KPSW)
  → Po sprzeciwie: rozprawa + obecność, możliwość obrony
```

### Uchylenie prawomocnego mandatu (art. 101 KPSW)
```
TERMIN: 7 dni od uprawomocnienia (ZAWITY)
  Wniosek: ukarany do sądu właściwego dla miejsca popełnienia
  Bez terminu: organ nakładający mandat / prokurator

PODSTAWY UCHYLENIA (art. 101 §1 i §1a):
  §1:   Czyn NIE jest wykroczeniem (np. nie wyczerpuje znamion)
  §1:   Sprawca nie ukończył 17 lat w chwili czynu
  §1:   Ustawowe wyłączenie odpowiedzialności (art. 15–17 KW)
  §1a:  Naruszenie zakazu z art. 96 §2 KPSW (np. powinno być inne postępowanie)
  §1a:  Kwota powyżej dopuszczalnej → uchylenie W CZĘŚCI (tylko nadwyżka)
  §1b:  Nowe fakty/dowody → sprawca nie popełnił czynu (nadzwyczajne)

⚠️ UCHYLENIE ≠ odwołanie:
  Sąd bada TYLKO podstawę formalną wystawienia (legalność)
  NIE bada winy ukaranego ani zasadności na nowo
```

---

## 4. ADMINISTRACYJNE KARY PIENIĘŻNE (KPA Dział IVa)

```
Podstawa: KPA Dz.U. 2025 poz. 1691 t.j. → Dział IVa art. 189a–189k

DYREKTYWY WYMIARU (art. 189d KPA — weryfikuj aktualną listę):
  Organ MUSI wziąć pod uwagę:
  □ Wagę i okoliczności naruszenia
  □ Częstotliwość / dotychczasowe naruszenia
  □ Uprzednie naruszenia (recydywa)
  □ Stopień zawinienia
  □ Działania podjęte po naruszeniu (naprawa, zaprzestanie)
  □ Współpracę z organem w toku postępowania
  □ Ważny interes publiczny

ULGI (art. 189k KPA):
  → Odstąpienie od nałożenia kary (pierwsze naruszenie, nieumyślność)
  → Rozłożenie na raty (max 36 mies. — weryfikuj)
  → Odroczenie terminu płatności
  → Umorzenie kary (art. 189l KPA)

TERMINY PRZEDAWNIENIA:
  → Wszczęcie postępowania: 5 lat od naruszenia (art. 189g §1 KPA — weryfikuj)
  → Nałożenie kary: weryfikuj art. 189g–189h KPA
  → Egzekucja: 5 lat od dnia, w którym kara stała się wymagalna (weryfikuj)

TRYB ZASKARŻENIA:
  Decyzja organu I stopnia → odwołanie (14 dni) → organ II stopnia / SKO
  → skarga do WSA (30 dni od decyzji SKO) → kasacja NSA (30 dni od wyroku WSA)
  ⚠️ WSTRZYMANIE: złóż wniosek o wstrzymanie wykonania decyzji (art. 61 §3 PPSA)!

WERYFIKUJ: isap.sejm.gov.pl → KPA → Dział IVa art. 189a–189k
```

---

## 5. EGZEKUCJA ADMINISTRACYJNA NIEZAPŁACONYCH GRZYWIEN/KAR

```
Podstawa: UPEA Dz.U. 2026 poz. 268 t.j. (weryfikuj nowelizacje)
Organ egzekucyjny: naczelnik US / wójt (burmistrz/prezydent) / inne

TRYB EGZEKUCJI NALEŻNOŚCI PIENIĘŻNYCH:
  → Tytuł wykonawczy (TW-1) wystawiany przez wierzyciela
  → Doręczenie TW zobowiązanemu
  → Środki: zajęcie rachunku bankowego, zajęcie wynagrodzenia, zajęcie wierzytelności,
    egzekucja z nieruchomości

UPRAWNIENIA ZOBOWIĄZANEGO:
  → Zarzuty (art. 33 UPEA): w ciągu 7 dni od doręczenia TW — najważniejszy środek
    Podstawy: wykonanie obowiązku, brak wymagalności, przedawnienie,
              błąd co do osoby, brak podstawy prawnej TW, brak doręczenia decyzji
  → Skarga na czynności egzekucyjne (art. 54 UPEA)
  → Wniosek o zawieszenie egzekucji

⚠️ TERMINY:
  Zarzuty: 7 dni od doręczenia TW (ZAWITY — art. 33 §1 UPEA)
  Skarga na czynności: 14 dni (weryfikuj aktualny termin)
```

---

## 6. GRZYWNY PORZĄDKOWE — PRZEGLĄD

```
W POSTĘPOWANIU ADMINISTRACYJNYM (KPA):
  Art. 88 KPA — świadek/biegły (50 zł / 200 zł) — zażalenie
  Art. 88a KPA — strona / uczestnik postępowania — weryfikuj w ISAP
  Art. 57 §4 KPA — wezwanie do usunięcia braków formalnych — weryfikuj

W POSTĘPOWANIU SĄDOWYM:
  KPC art. 163, 274 — świadek niestawiający się
  KPK art. 285–287 — świadek/biegły + tryb karny
  → Te są regulowane przez własne procedury (KPC/KPK)

W POSTĘPOWANIU WYKROCZENIOWYM:
  → Grzywna za niestawiennictwo świadka: KPSW art. 38
  → Weryfikuj aktualną kwotę w ISAP

Wniosek o uchylenie / umorzenie:
  → Składa się do organu nakładającego w terminie 7 dni od zawiadomienia
  → Zażalenie na odmowę: do organu wyższego stopnia w 7 dni
```

---

## 7. PRZEDAWNIENIE GRZYWIEN I KAR — ZESTAWIENIE

⚠️ NOWE 2026-06-13: pozycje "Opłata parkingowa (UDP)" i "Grzywna UPEA" w
tabeli niżej odwołują się do art. 70 Ordynacji podatkowej (przedawnienie
poprzez przepisy UPEA). PATRZ: `shared/ORKA-BAS-LEKSYKON.md` BAS-W32 —
nowelizacja OP znosi "wieczne przedawnienie" i wprowadza ugodę podatkową
od 01.10.2026 — może wpływać na te pozycje dla stanów po tej dacie.

```
TYP                        KARALNOŚĆ          WYKONANIE          ŹRÓDŁO
───────────────────────────────────────────────────────────────────────
Wykroczenie (KW)           1 rok od czynu     3 lata od wyroku   KW art. 45
                           2 lata gdy wszczęto
Mandat karny               Nie dotyczy        3 lata od prawom.  KW art. 45
Adm. kara pieniężna (KPA)  5 lat od narusz.   5 lat od wymagal.  KPA art. 189g
                                              (weryfikuj)
Opłata parkingowa (UDP)    Brak decyzji       5 lat (UPEA)       art. 70 OP
                                              (weryfikuj NSA)
Grzywna UPEA               —                  przedawnienie UPEA  art. 70 OP
                                              (weryfikuj)

⚠️ Wszystkie terminy weryfikuj w aktualnych tekstach ustaw — ZMIENIAJĄ SIĘ.
```

---

## 8. ORZECZNICTWO

```
Uchwała SN I KZP 7/25 z 19.03.2026:
  Prawomocny mandat za wykroczenie = res iudicata
  → wyklucza objęcie czynu czynem ciągłym art. 12 §2 KK
  → Weryfikuj sn.pl (uzasadnienie w przygotowaniu)

NSA II GSK 1206/23:
  Kara pieniężna za błędne dane kategorii pojazdu — weryfikuj orzeczenia.nsa.gov.pl

WSA Białystok II SA/Bk 159/24 z 28.05.2024:
  Opłata dodatkowa parkingowa NIE jest zaskarżalna wezwaniem do WSA
  → Można kwestionować TYLKO w postępowaniu egzekucyjnym (zarzuty do TW)
  → Weryfikuj: orzeczenia.nsa.gov.pl / orzeczenia.ms.gov.pl

NSA (linia):
  Obowiązek opłaty parkingowej i opłaty dodatkowej wynika BEZPOŚREDNIO Z PRAWA
  → Brak decyzji administracyjnej → brak postępowania adm.
  → Obrona dopiero w egzekucji (zarzuty do TW art. 33 UPEA)
  → Weryfikuj: orzeczenia.nsa.gov.pl → szukaj "strefa płatnego parkowania opłata dodatkowa"
```

---

## 9. QUALITY GATE

```
□ Czy wiadomo: grzywna sądowa / mandat / kara adm. / opłata parkingowa?
  (różne tryby obrony i terminy!)
□ Czy mandat jest prawomocny czy można jeszcze odmówić?
□ Termin na sprzeciw od wyroku nakazowego (7 dni) — czy nie upłynął?
□ Termin na uchylenie mandatu (7 dni) — czy nie upłynął?
□ Termin zarzutów egzekucyjnych (7 dni od TW) — czy nie upłynął?
□ Czy to kara adm. (KPA Dz. IVa) → sprawdź termin przedawnienia 5 lat
□ Czy opłata parkingowa → obrona TYLKO w egzekucji
□ Taryfikator mandatów zweryfikowany online (Dz.U. 2026 poz. 724)?
□ Kwota grzywny sądowej — weryfikuj art. 24 KW w ISAP (zmieniane)
```

---

## POWIĄZANIA

| Sytuacja | Skill / Moduł |
|---|---|
| Sprzeciw od wyroku nakazowego | `pisma-proste-v2` |
| Uchylenie mandatu art. 101 KPSW | `pisma-proste-v2` |
| Zarzuty do TW w egzekucji | `pisma-procesowe-v3` + `mod-UPEA-egzekucja-administracyjna` |
| Odwołanie od kary adm. (KPA) | `dr-05` → `mod-KPA-postepowanie-administracyjne` |
| Skarga do WSA na decyzję SKO | `dr-05` + `pisma-procesowe-v3` |
| Opłata parkingowa → egzekucja | sekcja poniżej + `mod-UPEA-egzekucja-administracyjna` |
| Kara mandatowa drogowa + pkt karne | `mod-PRD-prawo-jazdy-punkty-karne` |
| Grzywna porządkowa sądowa (KPC) | `mod-KPC-egzekucja-windykacja` |

---

## ŹRÓDŁA ONLINE

```
isap.sejm.gov.pl:
  → KW: WDU20250000734 (art. 24–37 grzywna; art. 45 przedawnienie)
  → KPSW: WDU20250000860 (art. 95–102 mandat; art. 101 uchylenie)
  → KPA: WDU20250001691 (art. 88 grzywna porządkowa; Dział IVa art. 189a–189k)
  → UPEA: WDU20230002505 (art. 119–125 grzywna przymuszenia; art. 33 zarzuty)
  → UDP: WDU20250000889 (art. 13, 13b, 13f opłaty parkingowe)

orzeczenia.nsa.gov.pl → "strefa płatnego parkowania opłata dodatkowa"
orzeczenia.ms.gov.pl  → "mandat uchylenie art 101 KPSW"
sn.pl                 → I KZP 7/25

web_search:
  "taryfikator mandatów 2026 rozporządzenie tabela"
  "uchylenie mandatu art 101 KPSW 7 dni termin wyrok NSA 2024 2025"
  "administracyjna kara pieniężna KPA art 189d przedawnienie 5 lat"
  "opłata dodatkowa parkingowa obrona egzekucja zarzuty 2024 2025"
```

---
*mod-grzywny-mandaty-szczegolowe.md · DR-03 · 2026-06-09*
*KW (Dz.U. 2025 poz. 734) + KPSW (Dz.U. 2025 poz. 860) + KPA (Dz.U. 2025 poz. 1691)*
*UPEA (Dz.U. 2023 poz. 2505) + UDP (Dz.U. 2025 poz. 889)*
